...
- use exact canonical syllabus wording unless an existing category-specific standard explicitly requires display capitalization cleanup
...
Current build enforcement may still be category-specific until the validation tooling is generalized. The standard in this document is the target even when a category is not yet fully enforced by code.
